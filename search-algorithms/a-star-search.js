    ...
    static search(game, source = 'S', target = 'G') {
        function ASearch(root, successor, goalTest, heuristic) {
            ...
            while (!priorQueue.isEmpty()) {
                const current = priorQueue.pop();
                const { node: currNode, path, g } = current;
                ...
                explored.add(currNode.value);

                const children = successor(currNode);
                for (const child of children) {
                    if (!explored.has(child.value)) {
                        const newG = g + 1; // Costo de movimiento = 1
                        const newF = newG + heuristic(child);
        ...
        function goalTest(node) {
            return node.value === target; // Corregido el "=" por "==="
        }
        ...
        function _heuristic(problem, goalPos) {
            return function heuristic(node) {
                ...
                const xDiference = Math.abs(nodePosition[0] - goalPos[0]);
                const yDiference = Math.abs(nodePosition[1] - goalPos[1]);
                const zDiference = Math.abs(nodePosition[2] - goalPos[2]);
    
                // Distancia Manhattan en 3D
                return xDiference + yDiference + zDiference;
            }
        }
